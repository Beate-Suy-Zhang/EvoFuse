...
def get_sod_train_loader(rgb_root, thermal_root, fus_root, gt_root, batchsize, trainsize, shuffle=True, num_workers=2, pin_memory=False, split='train'):
    if split == 'train':
...
class SalObjDataset_val(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # gt = gt.squeeze(0)  # Ensure gt is in the correct shape
        # print("gt shape222: ", gt.shape)
        # rgb, thermal, fus, gt, name = self.load_data()
